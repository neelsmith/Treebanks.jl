...
agldtlemmata = begin
	lemmata = []
	for sent in tb
		sentwords = map(w -> w.lemma, sent.words) |> unique
...
# ╔═╡ ebf42d64-24c4-47d6-9dea-23d54a376c67
# sortmycf = sort(resolve(s, pairs), by = triple -> triple[3], rev = true)

# ╔═╡ c2353b45-f201-4758-9524-0d83a9489971
begin
	alpha = ["a", "b", "c"]
	beta = ["d","e", "f", "g", "h"]
	vcat(alpha, beta)
...
md"""
*Set threshhold*  $(@bind thresh Slider(0:0.1:1.0, default = 0.8, show_value = true))
"""
...
begin
	map(agldtlemmata) do lemm
		resolve(lemm, pairs, threshhold = thresh)
	end
end

# ╔═╡ 0e1483bf-b21a-4661-b0d9-65bcbe1ae17e
resolve(agldtlemmata[100], pairs, threshhold = thresh)
